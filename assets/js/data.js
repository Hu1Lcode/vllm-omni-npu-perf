...
#   curl -L http://localhost:8091/v1/videos/{id}/content -o out.mp4`,
    ...
    perf: {
      columns: ["分辨率", "帧数 / 时长", "帧率 (fps)", "推理步数", "机型", "框架版本", "部署配置", "端到端时间 (s)", "每帧时间 (ms)", "备注"],
      rows: [],
    },
    refs: [
...
curl -sS -X POST "\${API_URL}" \\
  ...
  -o t2va.mp4`,
    ...
    perf: {
      columns: ["分辨率", "帧数 / 时长", "帧率 (fps)", "推理步数", "机型", "框架版本", "部署配置", "端到端时间 (s)", "每帧时间 (ms)", "备注"],
      rows: [["1344x768", "124 帧 / 5s", "24", "50", "Ascend910（64GB HBM/卡）", "vllm-omni v0.26.0 / CANN 9.0.1", "4× NPU，t2va", "437.85", "3531.0", "t2va, duration 5s, seed 1101；预热 1 次 + 3 次平均（服务端 e2e_total_ms）"]],
    },
    refs: [
...
# 响应 content[] 中每个元素对应一层图像（layers 默认 4）`,
    ...
    perf: {
      columns: ["分辨率", "层数", "推理步数", "机型", "框架版本", "部署配置", "单张耗时 (s)", "备注"],
      rows: [],
    },
    refs: [
      { label: "vLLM-Omni 文档 · 图像编辑 API", url: "https://docs.vllm.com.cn/projects/vllm-omni/en/latest/serving/image_edit_api/" },
