...
MEDIA_EXTS = {".mp4", ".webm", ".png", ".jpg", ".jpeg", ".gif"}
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        # /api/models/<id>/readme|script|perf|media
        api_m = re.fullmatch(r"/api/models/([^/]+)/(readme|script|perf|media)", self.path)
        if api_m:
            model_id, kind = api_m.group(1), api_m.group(2)
            if kind == "media":
                self._handle_media(model_id)
                return
    ...
    def _handle_media(self, model_id):
        """扫描 models/<id>/ 下的媒体文件，返回 JSON 列表。"""
        if not MODEL_ID_RE.fullmatch(model_id):
            self.send_error(400, "invalid model id")
            return
        model_dir = os.path.join(MODELS_DIR, model_id)
        if not os.path.isdir(model_dir):
            self.send_error(404, "model directory not found")
            return
        files = []
        try:
...
def main():
    ...
    print(f"服务已启动: http://{args.bind}:{args.port}/  （模型内容同步: /api/models/<模型id>/readme|script|perf|media）", flush=True)
    try:
